...
sys.path.append("../../../../../")
## param
from params import Temp_FL_IW_SVHNParamParser as ParamParser
## model
...
loadSrc = lambda batch_size: loadSVHN('datasets/SVHN', batch_size, train_shuffle=False, val_shuffle=False)
loadTar = lambda batch_size: loadMNIST('datasets/MNIST', batch_size, 32, gray=False, train_shuffle=False, val_shuffle=False)
##--------------------------------------------------


def plot_box(iws, save_root, font_size):
...
if __name__ == "__main__":
    ...
    if not os.path.exists(iws_fn):
        ...
        # save
        pickle.dump((iws_exps_tr, iws_exps_val), open(iws_fn, "wb"))
    else:
        iws_exps_tr, iws_exps_val = pickle.load(open(iws_fn, "rb"))
    
    ## print and plot
    iws_exps_tr = iws_exps_tr.t() + 0.5 - 1.0
    iws_exps_val = iws_exps_val.t() + 0.5 - 1.0
